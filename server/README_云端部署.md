...
多个用户填**同一个地址 + 同一把 API Key** 就能一起用，服务端自动**人人平等轮流**：
- 采用加权公平队列（等权）：一次只跑一句（单卡不抢），并在用户间轮转——**一个用户狂发也占不满、别人不饿死**；
- 客户端自动上报 `X-User-Id`（每台安装稳定唯一）区分用户，无需各自配不同 Key；
- 单用户在队上限默认 6，可用环境变量 `DOTS_MAX_INFLIGHT_PER_USER` 调整（超过返回 429，客户端提示"排队已满，稍后再试"）；
- `/health` 会返回 `queued`（排队请求数）、`waiting_users`（排队用户数）便于观察。

> 想给个别用户更高优先级（加权而非等权）可后续扩展；当前默认「人人平等」。
