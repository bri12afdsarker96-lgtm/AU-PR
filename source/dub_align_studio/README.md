...
云配版口径（`MERCURY_CLOUD_ONLY=1`）：引擎下拉曝光 `dots_remote + edge_tts`，
本地模型组件（dots_local/fish_local/torch）隐藏。顶栏 ☁ 云 GPU 状态灯**只代表
dots_remote**——选 Edge TTS 时不会触发云 GPU 唤醒/关机/空闲监听。
