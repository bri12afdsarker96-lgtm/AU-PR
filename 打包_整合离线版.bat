...
rem [附] 拷贝云配音部署脚本 server\（带上就能在目标机一键部署云 GPU；没有则跳过）
if exist "server" (
  echo [附] 拷贝 server\（云配音部署脚本）...
...
rem [附] 自动打包 ffmpeg/ffprobe（找到就带上，目标机渲染开箱即用，免手动放）
set "FFM="
set "FFP="
...
if not defined FFM if exist "dist\水星配音对齐工作室\ffmpeg.exe" set "FFM=dist\水星配音对齐工作室\ffmpeg.exe"
if not defined FFP if exist "dist\水星配音对齐工作室\ffprobe.exe" set "FFP=dist\水星配音对齐工作室\ffprobe.exe"
...
if defined FFM if defined FFP (
  echo [附] 打包 ffmpeg/ffprobe（渲染开箱即用）...
  copy /y "%FFM%" "%PKG%\ffmpeg.exe" >nul
  copy /y "%FFP%" "%PKG%\ffprobe.exe" >nul
  set "FFBUNDLED=1"
) else (
  echo [附] 未找到 ffmpeg/ffprobe，未打包（目标机需手动放 ffmpeg.exe/ffprobe.exe 到包内或 PATH）。
)
...
if defined FFBUNDLED (echo        ffmpeg/ffprobe 已随包，渲染开箱即用。) else (echo        （渲染成片需 ffmpeg：把 ffmpeg.exe/ffprobe.exe 放进该文件夹或系统 PATH。）)
echo.
