...
def write_build_info(dist_dir: Path, exe_name: str) -> Path:
    """生成 _build_info.py 并塞进 pyinstaller onedir 的 _internal 里。

    每次打包生成**新的**随机密钥：
      - INTEGRITY_HMAC_KEY：算 exe HMAC 的 key，运行时 rasp.integrity_check 用
      - XOR_SEED：crypto_store 本地加密 XOR 种子
      - EXE_HMAC_HEX：HMAC-SHA256(exe, INTEGRITY_HMAC_KEY) 的结果，
        作为 rasp.integrity_check 的 baseline

    整个 _build_info.py 会被 pyinstaller 打成 .pyc（编译过的字节码），
    随后如果流程加了 Cython 编译，还会变成 .pyd/.so（更难反）。
    """
    import secrets
    from datetime import datetime

    _internal = dist_dir / "_internal" / "dub_align_studio"
    if not _internal.exists():
        _log(f"[!] {_internal} 不存在——build_info 写不进去，跳过")
        return Path()
    ...
    # 用 integrity_key 算 exe HMAC 作为 baseline
    exe_hmac = write_integrity_hash(dist_dir, exe_name, hmac_key=integrity_key)

    ts = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")

    content = (
        "# -*- coding: utf-8 -*-\n"
        "# GENERATED by build_dist.write_build_info() —— DO NOT EDIT\n"
        "# 每次打包重新生成，密钥每次不同\n"
        "PACKAGED = True\n"
        f"BUILD_TIMESTAMP = {ts!r}\n"
        f"INTEGRITY_HMAC_KEY = {integrity_key!r}\n"
        f"XOR_SEED = {xor_seed!r}\n"
        f"EXE_HMAC_HEX = {exe_hmac!r}\n"
        "SERVER_URL_ENC = ''  # 保留位；后续用 XOR_SEED 加密 DEFAULT_SERVER 后填入\n"
        "APP_ID_ENC = ''\n"
    )
    target = _internal / "_build_info.py"
    target.write_text(content, encoding="utf-8")
    _log(f"[OK] _build_info.py 写入 {target}")
    _log(f"     PACKAGED=True  integrity_key=<32B>  xor_seed=<32B>  exe_hmac={exe_hmac[:16]}...")
    return target
